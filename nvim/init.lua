...
map("n", "<leader>cc", function()
    ...
    vim.fn.system("pane=$(tmux list-panes -a -F '#{pane_id} #{pane_current_command}' | awk '/claude/{print $1; exit}'); if [ -n \"$pane\" ]; then tmux switch-client -t \"$pane\" 2>/dev/null || tmux select-pane -t \"$pane\"; else tmux split-window -h -l 90 -c '" .. cwd .. "' 'bash -li -c claude'; fi")
